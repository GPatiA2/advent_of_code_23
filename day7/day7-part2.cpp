...
private:
  ...
  Hand_Val get_hand_val() {
    ...
    if (card_type_counter.find('J') != card_type_counter.end()) {
      int num_jokers = card_type_counter['J'];
      cout << "FOUND " << num_jokers << " IN HAND " << cards << endl;
      card_type_counter.erase('J');
      char best_card = find_best_card(card_type_counter);
      card_type_counter[best_card] += num_jokers;
      cout << "ADDING THEM TO BEST CARD " << best_card << endl;
    }
  ...
  char find_best_card(const map<char, int> &m) {
    auto it = m.begin();
    char best_card = it->first;
    ...
    while (it != m.end()) {
      if (it->second > amount) {
        ...
        amount = it->second;
                 ...
                 card_val(it->first) < card_val(best_card)) {
        best_card = it->first;
        amount = it->second;
      }
      it++;
...
vector<char> Hand::card_order = {'A', 'K', 'Q', 'T', '9', '8', '7',
                                 '6', '5', '4', '3', '2', 'J'};
