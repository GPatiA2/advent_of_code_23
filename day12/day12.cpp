...
struct spring_row {
  ...
  Matrix<long int> memo;

  spring_row(string s, vector<int> &v) {
    status = s;
    damage_groups = v;
    memo = Matrix<long int>(status.size(), damage_groups.size(), -1);
  }
  ...
  long int arrangements(long int string_idx, long int groups_idx,
    ...
    if (memo[string_idx][groups_idx] != -1) {
      return memo[string_idx][groups_idx];
    }
    ...
    memo[string_idx][groups_idx] = res;
    return res;
  }
};
